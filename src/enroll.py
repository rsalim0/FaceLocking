...
def main():
    ...
    # Accept name from command line argument or prompt
    if len(sys.argv) > 1:
    ...
    # Auto-enable auto-capture if name provided via command line
    auto = len(sys.argv) > 1
    last_auto = 0.0
    if auto:
        status_msg = "Auto-capture enabled (press 'a' to toggle)"
    ...
    try:
        while True:
            ...
            if total >= cfg.samples_needed and len(new_samples) > 0:
                ...
                save_db(cfg, db, meta)
                status_msg = f"AUTOSAVED '{name}' (Target {cfg.samples_needed} met)"
